...
def find_optimal_route(city_name, points, start_point = None, route_type = 'drive', plot_route = True):
    ...
    try:
        ...
        print("Adreses ģeokodēšana...")
        # Visus punktus koordinates
        coordinates = []
        for point in points:
        ...
        # Grafika tuvāko mezglu atrašana
        nodes = []
        ...
        # Laikas matricas izveidošana starp visiem punktiem
        n = len(points)
        ...
        # Optimālas secību meklēšana
        point_indices = list(range(n))
        
        if start_point is not None:
            # Jā ir starta punkts, fiksējam
            ...
            # Jā nav starta punkta, izskatām visas iespējamas adreses
            permutations_list = list(permutations(point_indices))
        
        # Cēļa min laika meklēšana
        min_time = float('inf')
        ...
        # Pedēja mezgla pievienošana
        full_route_nodes.append(nodes[best_route_indices[-1]])
        
        # Kopējā attāluma aprēķināšana
        total_distance = 0
        for k in range(len(best_route_indices)-1):
            i, j = best_route_indices[k], best_route_indices[k + 1]
    ...
    # Optimālo maršrūtu vizualizēšana
